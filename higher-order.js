...
//Code Here
let myStrongest = monstersInYourPocket.filter(monsterObj => {
...
let orderTotals = orders.map(totalObj => {
  // console.log(element)
  return totalObj.price * (totalObj.tax + 1)
});
...
/*
  map over the purchases array DONE
  check if the owner is bob DONE
  if yes, then add price to acc DONE
  else nothing DONE
*/

let bobsTotal = purchases.reduce((acc, purchase, i, arr) => {
